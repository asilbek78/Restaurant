...
function Footer() {
  ...
  const { data: three, isLoading } = UseFetch("https://omvwbniuooqqvunluhkz.supabase.co/storage/v1/object/sign/db/db.json?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1cmwiOiJkYi9kYi5qc29uIiwiaWF0IjoxNzQyODg4ODk2LCJleHAiOjE3NzQ0MjQ4OTZ9.NV0T1LNduVjHMtwonR04cVgRfKQSXTw0O52a-Qp5izE");
  return (
    <>
      ...
      <section className=" bg-[#3b3b3b] w-full dark:bg-gray-800">
        <div  className=" container w-full h-[280px] flex justify-around items-center transition duration-300">
          <div className="flex flex-col gap-4 yuq">
            <h1 className="text-xl font-bold text-yellow-400">{t("footers.h1")}</h1>
            <h4 className=" text-[15px] max-w-[300px] text-gray-100 font-sans">
              {t("footers.h4")}
            </h4>
          </div>
          <div className="w-[200px] flex flex-col gap-4">
            <h1 className="text-xl font-bold text-yellow-400 e">{t("footers.h21")}</h1>
            <ul className="uls text-[15px] text-gray-100 font-semibold flex flex-col gap-1">
              <Link to={"/uzbek"}>
          ...
          {three?.footer.map((item, id) => (
            <div key={id} className="flex gap-10 bigdiv">
              <div className=" divs flex flex-col gap-[11px]">
                ...
                <article className="flex items-center gap-2 cursor-pointer group ">
                  <Send className="text-white duration-200 group-hover:text-gray-400" size={24} />
              ...
              <div className="w-[320px] h-[190px] xarita">
                <iframe
